...
def estimateVelocities_3d(detections,detections_3d,searchRangeFrames,frame_start,frame_end,nearest_neighbors,speedLimit):
    estimatedVelocities =[]

    ###Compute all pairwise distances 
    pairDistance = pairwise_distances(detections_3d,metric='euclidean')
    ...
    for currentDtectionIndex in range(0,numDetections):
        ...
        for frame in range(currentFrame-nearest_neighbors,currentFrame+nearest_neighbors):
           ...
           # print('detectionsAtThisTimeInstant',targetDetectionIndex)
            estimatedVelocity = detections_3d[targetDetectionIndex] - detections_3d[currentDtectionIndex]
        ...
        estimatedVelocities.append([np.mean(velocities[:,0]),np.mean(velocities[:,1]),np.mean(velocities[:,2])])

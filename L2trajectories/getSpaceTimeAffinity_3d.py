...
from L2trajectories import getTrackletFeatures_3d
...
def getSpaceTimeAffinity_3d(tracklets, beta1, speedLimit, indifferenceLimit):
...
#    velocity = []   ### [[vx,vy],[vx,vy],....]
    ...
    startpoint_3d,endpoint_3d, intervals, duration, velocity_3d = getTrackletFeatures_3d.getTrackletFeatures_3d(tracklets)
    ...
    startpoint_3d = np.array(startpoint_3d)
    endpoint_3d = np.array(endpoint_3d)
    centers = 0.5*(startpoint_3d+endpoint_3d)
    ...
    centersDistance_mer[centersDistance_mer < 5] = 1      ###  need rethink
    ...
    errorZForward = endZ + np.multiply(velocityZ,frameDifference) - np.transpose(startZ)
    ...
    errorZBackward = np.transpose(startZ) + np.multiply(np.transpose(velocityZ),-np.transpose(frameDifference)) - endZ
    ...
    zDiff = endZ - np.transpose(startZ)
